...
class Graph:
    ...
    def test(self, image: Image,
             parts: Optional[List[str]] = None) -> TestResult:
        ...
        return TestResult(positions={n: None if p is None else index_tx(p)
                                     for n, p in inf_result.positions.items()},
                          candidates={n: index_tx(c)
    ...
    def prepare_image(self, image_itk: sitk.Image) \
        ...
        if orientation != self.orientation:
            log.info('Re-orienting image from %s to %s', orientation,
                     self.orientation)
            image_itk = hiwi.change_anatomical_orientation(image_itk,
                                                           self.orientation)

        # resampling, if necessary
        if (np.abs(image_itk.GetSpacing()[::-1] - spacing) > 0.01).any():
    ...
    def infer(self, image: Image, candidates: Dict[str, np.ndarray],
        ...
        t = time()
        energy_graph = self.compute_energies(image, candidates, use_weights,
                                             support_unknown)
        time_energies = time() - t

        t = time()
        state, energy = solver.infer(energy_graph)
    ...
    def compute_energies(self, image: Image, candidates: Dict[str, np.ndarray],
        ...
        for potential in self.potentials:
            ...
            if len(combinations) == 0:
                ...
                values = potential.compute(image, positions)
